...
redis.on('connect', () => console.log('API connected to Redis'));
...
export const getAllBuildings = async (): Promise<string[]> => {
    const keys = await redis.keys('building:*:tenants');
    const buildings = keys.map(key => {
        const match = key.match(/building:(.+):tenants/);
